...
def generate_postal_code(postal_code_mask, pattern_word_char_dic):
    ...
    for pattern_word, pattern_char in pattern_word_char_dic.items():
        ...
        pattern_word_idx_list = find_all(postal_code_mask_low, pattern_word)
        ...
        for i in range(len(pattern_word_idx_list) - 1):
            ...
            postal_code_mask_normalized += postal_code_mask_low[current_idx + len(pattern_word) : next_idx] 

        postal_code_mask_normalized += pattern_char + postal_code_mask_low[last_idx + len(pattern_word):]
    
        print(postal_code_mask_normalized)
        postal_code_mask_low = postal_code_mask_normalized
